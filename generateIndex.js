...
const hasCompany = config.company && config.company.length > 0;
...
module.exports = (archiveName = null) => {
  ...
  // output html
  const html = `<html><head><title>${pkg.description}${
    hasCompany ? ` | ${config.company}` : ``
  }</title><style>${css}</style></head><body><div id="wrap"><header><h1>${
    pkg.description
